...
import { ArrowLeft, FileText, Calendar, BookOpen, FolderOpen } from 'lucide-react'
import { getCategories, getArticles, getCategoryFromId, getSubCategories, getSubCategoryName } from '@/lib/blog'
...
export default async function CategoryPage({ params, searchParams }: PageProps) {
  ...
  const subCategories = await getSubCategories(decodedCategory)
  ...
  // 按子目录分组文章
  const articlesBySubCategory: Record<string, typeof articles> = {}
  paginatedArticles.forEach(article => {
    const key = article.subCategory || ''
    if (!articlesBySubCategory[key]) {
      articlesBySubCategory[key] = []
    }
    articlesBySubCategory[key].push(article)
  })
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                      共 {articles.length} 篇文章{subCategories.length > 0 && ` · ${subCategories.length} 个子目录`}
                    </p>
                  </div>
                </div>
                {/* 视频教程按钮 */}
            ...
            {/* 按子目录分组显示文章 */}
            {Object.entries(articlesBySubCategory).map(([subCategoryPath, subArticles]) => (
              <div key={subCategoryPath} className="mb-8">
                {subCategoryPath && (
                  <div className="flex items-center gap-2 mb-4 border-b border-border pb-2">
                    <FolderOpen className="h-5 w-5 text-primary" />
                    <h2 className="text-lg font-semibold text-foreground">
                      {getSubCategoryName(subCategoryPath)}
                    </h2>
                    <span className="text-sm text-muted-foreground">
                      ({subArticles.length} 篇)
                    </span>
                  </div>
                )}
