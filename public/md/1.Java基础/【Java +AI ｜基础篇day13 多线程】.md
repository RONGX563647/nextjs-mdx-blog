##### 13🧵 Java Day13 - 多线程

> 💡 **核心提示**：多线程是 Java 并发编程的基础，本文系统讲解线程创建、线程同步、线程池等核心概念，帮你掌握 Java 多线程编程。

---

#### 快速回顾

- **线程**：程序执行的最小单元，进程中的单个执行流
- **线程创建**：继承 Thread、实现 Runnable、实现 Callable
- **线程同步**：synchronized、Lock、原子类，解决线程安全问题
- **线程池**：管理和复用线程，提高性能和资源利用率

---

#### 目录

- [一、线程基础](#一线程基础)
  - [1. 线程与进程](#1-线程与进程)
  - [2. 线程创建方式](#2-线程创建方式)
  - [3. 线程常用方法](#3-线程常用方法)
- [二、线程同步](#二线程同步)
  - [1. 线程安全问题](#1-线程安全问题)
  - [2. synchronized 关键字](#2-synchronized-关键字)
  - [3. Lock 接口](#3-lock-接口)
  - [4. 原子类](#4-原子类)
- [三、线程间通信](#三线程间通信)
- [四、线程池](#四线程池)
  - [1. 为什么要用线程池](#1-为什么要用线程池)
  - [2. 创建线程池](#2-创建线程池)
  - [3. 线程池参数详解](#3-线程池参数详解)
  - [4. 线程池使用](#4-线程池使用)
- [五、线程状态](#五线程状态)
- [问答](#问答)

---

#### 详细内容

##### 一、线程基础

#### 1. 线程与进程

| 特性 | 进程 | 线程 |
|------|------|------|
| 定义 | 程序的一次执行 | 进程中的执行单元 |
| 资源 | 独立地址空间 | 共享进程资源 |
| 开销 | 大（创建/切换） | 小 |
| 通信 | 复杂（IPC） | 简单（共享内存） |
| 崩溃影响 | 不影响其他进程 | 可能导致整个进程崩溃 |

#### 2. 线程创建方式

**方式1：继承 Thread 类**

```java
public class MyThread extends Thread {
    @Override
    public void run() {
        for (int i = 0; i < 100; i++) {
            System.out.println(Thread.currentThread().getName() + ": " + i);
        }
    }
}

// 使用
MyThread t1 = new MyThread();
t1.setName("线程1");
t1.start();  // 启动线程，调用 run() 方法
```

**方式2：实现 Runnable 接口（推荐）**

```java
public class MyRunnable implements Runnable {
    @Override
    public void run() {
        for (int i = 0; i < 100; i++) {
            System.out.println(Thread.currentThread().getName() + ": " + i);
        }
    }
}

// 使用
MyRunnable runnable = new MyRunnable();
Thread t2 = new Thread(runnable, "线程2");
t2.start();

// Lambda 简化
new Thread(() -> {
    System.out.println("新线程执行");
}, "线程3").start();
```

**方式3：实现 Callable 接口（有返回值）**

```java
public class MyCallable implements Callable<String> {
    @Override
    public String call() throws Exception {
        Thread.sleep(1000);
        return "任务完成";
    }
}

// 使用
FutureTask<String> futureTask = new FutureTask<>(new MyCallable());
new Thread(futureTask).start();
String result = futureTask.get();  // 阻塞等待结果
System.out.println(result);
```

#### 3. 线程常用方法

```java
Thread t = new Thread();

// 启动线程
t.start();

// 线程休眠（不会释放锁）
Thread.sleep(1000);

// 线程让步（让出 CPU，进入就绪状态）
Thread.yield();

// 等待该线程执行完毕
t.join();

// 设置守护线程
t.setDaemon(true);

// 获取线程信息
String name = t.getName();
int priority = t.getPriority();  // 1-10，默认5
Thread.State state = t.getState();
boolean isAlive = t.isAlive();
boolean isDaemon = t.isDaemon();

// 中断线程
t.interrupt();
boolean isInterrupted = t.isInterrupted();
```

---

##### 二、线程同步

#### 1. 线程安全问题

```java
public class TicketSystem {
    private int tickets = 100;
    
    // 线程不安全
    public void sellTicket() {
        if (tickets > 0) {
            System.out.println(Thread.currentThread().getName() + 
                "卖出第" + tickets + "张票");
            tickets--;
        }
    }
}
```

**问题**：多个线程同时判断 `tickets > 0`，可能导致超卖。

#### 2. synchronized 关键字

**同步代码块**：
```java
public class TicketSystem {
    private int tickets = 100;
    private final Object lock = new Object();
    
    public void sellTicket() {
        synchronized (lock) {  // 同步代码块
            if (tickets > 0) {
                System.out.println(Thread.currentThread().getName() + 
                    "卖出第" + tickets + "张票");
                tickets--;
            }
        }
    }
}
```

**同步方法**：
```java
public class TicketSystem {
    private int tickets = 100;
    
    public synchronized void sellTicket() {  // 同步方法
        if (tickets > 0) {
            System.out.println(Thread.currentThread().getName() + 
                "卖出第" + tickets + "张票");
            tickets--;
        }
    }
}
```

**静态同步方法**：
```java
public class TicketSystem {
    private static int tickets = 100;
    
    public static synchronized void sellTicket() {  // 类锁
        if (tickets > 0) {
            tickets--;
        }
    }
}
```

#### 3. Lock 接口

```java
import java.util.concurrent.locks.ReentrantLock;

public class TicketSystem {
    private int tickets = 100;
    private final ReentrantLock lock = new ReentrantLock();
    
    public void sellTicket() {
        lock.lock();  // 获取锁
        try {
            if (tickets > 0) {
                System.out.println(Thread.currentThread().getName() + 
                    "卖出第" + tickets + "张票");
                tickets--;
            }
        } finally {
            lock.unlock();  // 释放锁（必须在 finally 中）
        }
    }
}
```

**synchronized vs Lock**：

| 特性 | synchronized | Lock |
|------|--------------|------|
| 实现方式 | JVM 层面 | API 层面 |
| 释放锁 | 自动释放 | 手动释放（finally） |
| 可中断 | 不支持 | 支持 `lockInterruptibly()` |
| 超时获取 | 不支持 | 支持 `tryLock(timeout)` |
| 条件变量 | 一个（wait/notify） | 多个（Condition） |
| 公平锁 | 非公平 | 可配置公平/非公平 |

#### 4. 原子类

```java
import java.util.concurrent.atomic.AtomicInteger;

public class Counter {
    private AtomicInteger count = new AtomicInteger(0);
    
    public void increment() {
        count.incrementAndGet();  // 原子自增
    }
    
    public int getCount() {
        return count.get();
    }
}
```

**常用原子类**：
- `AtomicInteger`、`AtomicLong`、`AtomicBoolean`
- `AtomicIntegerArray`、`AtomicLongArray`
- `AtomicReference`

---

##### 三、线程间通信

```java
public class ProducerConsumer {
    private boolean flag = false;
    private String data;
    
    // 生产者
    public synchronized void produce(String value) {
        while (flag) {
            try {
                wait();  // 等待消费
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }
        data = value;
        flag = true;
        notifyAll();  // 通知消费者
    }
    
    // 消费者
    public synchronized String consume() {
        while (!flag) {
            try {
                wait();  // 等待生产
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }
        flag = false;
        notifyAll();  // 通知生产者
        return data;
    }
}
```

**关键方法**：
- `wait()`：释放锁，进入等待状态
- `notify()`：唤醒一个等待线程
- `notifyAll()`：唤醒所有等待线程

---

##### 四、线程池

#### 1. 为什么要用线程池

- **降低资源消耗**：复用线程，减少创建/销毁开销
- **提高响应速度**：任务到达时无需等待线程创建
- **便于管理**：控制并发线程数，便于监控

#### 2. 创建线程池

```java
import java.util.concurrent.*;

// 方式1：Executors 工厂方法（不推荐，有OOM风险）
ExecutorService pool1 = Executors.newFixedThreadPool(10);
ExecutorService pool2 = Executors.newCachedThreadPool();
ExecutorService pool3 = Executors.newSingleThreadExecutor();

// 方式2：ThreadPoolExecutor 手动创建（推荐）
ThreadPoolExecutor executor = new ThreadPoolExecutor(
    5,                      // 核心线程数
    10,                     // 最大线程数
    60L,                    // 空闲线程存活时间
    TimeUnit.SECONDS,       // 时间单位
    new LinkedBlockingQueue<>(100),  // 任务队列
    Executors.defaultThreadFactory(), // 线程工厂
    new ThreadPoolExecutor.CallerRunsPolicy()  // 拒绝策略
);
```

#### 3. 线程池参数详解

| 参数 | 说明 |
|------|------|
| corePoolSize | 核心线程数，即使空闲也保留 |
| maximumPoolSize | 最大线程数 |
| keepAliveTime | 非核心线程空闲存活时间 |
| workQueue | 任务等待队列 |
| threadFactory | 创建线程的工厂 |
| handler | 拒绝策略 |

**拒绝策略**：
- `AbortPolicy`：直接抛出异常（默认）
- `CallerRunsPolicy`：由调用线程执行任务
- `DiscardPolicy`：静默丢弃任务
- `DiscardOldestPolicy`：丢弃队列最老的任务

#### 4. 线程池使用

```java
// 提交任务
executor.execute(() -> System.out.println("任务1"));

// 提交有返回值的任务
Future<Integer> future = executor.submit(() -> 42);
Integer result = future.get();

// 批量提交
List<Callable<Integer>> tasks = Arrays.asList(
    () -> 1, () -> 2, () -> 3
);
List<Future<Integer>> futures = executor.invokeAll(tasks);

// 关闭线程池
executor.shutdown();  // 优雅关闭，等待任务完成
// executor.shutdownNow();  // 立即关闭，返回未执行任务
```

---

##### 五、线程状态

```
NEW（新建）→ start() → RUNNABLE（可运行）→ wait() → WAITING（等待）
                              ↓                      ↓ notify()
                         sleep() → TIMED_WAITING（超时等待）
                              ↓
                         获取锁失败 → BLOCKED（阻塞）
                              ↓
                         run()结束 → TERMINATED（终止）
```

**线程生命周期状态图：**

```
┌─────────────────────────────────────────────────────────────┐
│              Java 线程生命周期状态详解        │
│                                                             │
│   ┌─────────────────────────────────────────────┐           │
│   │        NEW（新建状态）                 │           │
│   │                                             │           │
│   │   • 创建线程对象，未调用 start()              │           │
│   │   • Thread t = new Thread()               │           │
│   └──────────────────┬──────────────────────────┘           │
│                      │                                       │
│                      │ 调用 start()                          │
│                      ▼                                       │
│   ┌─────────────────────────────────────────────┐           │
│   │     RUNNABLE（可运行状态）             │           │
│   │                                             │           │
│   │   • 调用 start() 后进入此状态                │           │
│   │   • 等待 CPU 分配时间片                     │           │
│   │   • 可能正在执行，也可能在等待 CPU              │           │
│   └──────────────────┬──────────────────────────┘           │
│                      │                                       │
│          ┌───────────┴───────────┐                           │
│          ▼                       ▼                           │
│   ┌──────────────────┐   ┌──────────────────┐           │
│   │   BLOCKED       │   │   WAITING       │           │
│   │   阻塞状态       │   │   等待状态       │           │
│   │                 │   │                 │           │
│   │   • 等待锁       │   │   调用 wait()   │           │
│   │   • synchronized │   │   • 调用 join()  │           │
│   │     阻塞         │   │   • 调用         │           │
│   │                 │   │   Lock.lock()    │           │
│   └────────┬─────────┘   └────────┬─────────┘           │
│            │                      │                       │
│            │ 获得锁               │ notify()               │
│            │                      │ notifyAll()            │
│            └──────────┬───────────┘                       │
│                       │                                     │
│                       ▼                                     │
│   ┌─────────────────────────────────────────────┐           │
│   │   RUNNABLE（重新可运行）              │           │
│   └──────────────────┬──────────────────────────┘           │
│                      │                                       │
│                      │ sleep(time)                          │
│                      ▼                                       │
│   ┌─────────────────────────────────────────────┐           │
│   │  TIMED_WAITING（超时等待）            │           │
│   │                                             │           │
│   │   • Thread.sleep(1000)                  │           │
│   │   • t.join(1000)                        │           │
│   │   • Lock.tryLock(1000)                 │           │
│   │   • 超时后自动返回 RUNNABLE            │           │
│   └──────────────────┬──────────────────────────┘           │
│                      │                                       │
│                      │ 超时 / 被中断                          │
│                      ▼                                       │
│   ┌─────────────────────────────────────────────┐           │
│   │   RUNNABLE（重新可运行）              │           │
│   └──────────────────┬──────────────────────────┘           │
│                      │                                       │
│                      │ run() 方法执行完毕                     │
│                      ▼                                       │
│   ┌─────────────────────────────────────────────┐           │
│   │   TERMINATED（终止状态）               │           │
│   │                                             │           │
│   │   • 线程执行完成                          │           │
│   │   • 调用 stop()（已废弃）                   │           │
│   │   • 抛出未捕获异常                        │           │
│   │   • 不能再启动                              │           │
│   └─────────────────────────────────────────────┘           │
│                                                             │
│   状态转换总结：                                              │
│   • NEW → RUNNABLE：调用 start()                          │
│   • RUNNABLE → BLOCKED：等待 synchronized 锁               │
│   • RUNNABLE → WAITING：调用 wait()、join() 等            │
│   • RUNNABLE → TIMED_WAITING：调用 sleep()、join(time) 等   │
│   • BLOCKED/WAITING/TIMED_WAITING → RUNNABLE：获得锁/超时/被唤醒│
│   • RUNNABLE → TERMINATED：run() 方法执行完毕                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

#### 问答

##### Q1：start() 和 run() 有什么区别？

**答**：
- `start()`：启动新线程，JVM 调用 `run()` 方法，异步执行。
- `run()`：普通方法调用，在当前线程执行，不会创建新线程。
- **注意**：直接调用 `run()` 不会实现多线程效果。

##### Q2：synchronized 和 Lock 如何选择？

**答**：
- **synchronized**：简单场景，代码简洁，自动释放锁。
- **Lock**：需要高级功能（可中断、超时、多条件），手动控制锁。
- **建议**：优先使用 synchronized，不满足需求时再用 Lock。

##### Q3：wait() 和 sleep() 有什么区别？

**答**：
| 特性 | wait() | sleep() |
|------|--------|---------|
| 所属类 | Object | Thread |
| 锁释放 | 释放 | 不释放 |
| 使用场景 | 线程通信 | 暂停执行 |
| 唤醒方式 | notify()/notifyAll() | 时间到自动唤醒 |
| 调用位置 | 同步代码块内 | 任意位置 |

##### Q4：线程池的核心线程数如何设置？

**答**：
- **CPU 密集型**：核心线程数 = CPU 核心数 + 1
- **IO 密集型**：核心线程数 = CPU 核心数 × 2 或更高
- **混合型**：根据实际情况测试调整
- **公式**：`线程数 = CPU 核心数 × (1 + 等待时间 / 计算时间)`

##### Q5：什么是线程死锁？如何避免？

**答**：
- **死锁**：多个线程互相等待对方释放锁，导致永久阻塞。
- **产生条件**：互斥、占有且等待、不可抢占、循环等待。
- **避免方法**：
  - 按固定顺序获取锁
  - 使用超时机制（tryLock）
  - 减少锁的持有时间
  - 使用并发工具类（ConcurrentHashMap 等）

---

> **学习建议**：多线程是 Java 高级编程的核心，也是面试重点。务必理解线程同步的原理，掌握 synchronized 和 Lock 的使用，熟练使用线程池。建议多写练习，特别是生产者消费者等经典模型。
